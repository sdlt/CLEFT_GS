...
void interpXi(double p[])
{
  ...
  for (i=0;i<nrs;i++) xi[i] = data[2][i] + p[1]*data[3][i] + p[2]*data[4][i] + p[1]*p[1]*data[5][i] + p[1]*p[2]*data[6][i] + p[2]*p[2]*data[7][i]
      + p[3]*data[8][i] + p[3]*p[3]*data[9][i] + p[1]*p[3]*data[10][i] + p[2]*p[3]*data[11][i] + p[4]*data[13][i] + p[1]*p[4]*data[14][i] + p[8]*data[12][i];
...
void interpV12(double p[])
{
  ...
  for (i=0;i<nrs;i++) v12[i] = data[15][i] + p[1]*data[16][i] + p[2]*data[17][i] + p[1]*p[1]*data[18][i] + p[1]*p[2]*data[19][i] + p[3]*data[23][i] + p[1]*p[3]*data[24][i] + p[4]*data[22][i] + p[9]*data[20][i] + p[10]*data[21][i];
...
double fXis (double y, double p[])
{
  ...
  if (var>0) return (1.+xi_r)*gauss(x, mean, var);
  else return 0;
}
...
void load_CLEFT(char *file)
{
  ...
  for (i=0; i<nc; i++) data[i] = (double *)malloc(sizeof(double)*n);
  ...
  if (fi) {
    ...
    for(i=0; i<n; i++) for(j=0; j<nc; j++) fscanf(fi,"%lf",&data[j][i]);
  }
  fclose(fi);
